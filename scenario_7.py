...
def camera_callback(image, data_dict):
    # Make a black cluster to act as an obstruction
    i = np.array(np.copy(image.raw_data))
    i_reshape = np.reshape(np.copy(i), (image.height, image.width, 4))
    # The starting and stopping coordinates.
    start, stop = -280, 250
    #start, stop = 250, -450
    cluster = [0,0,0,255]
    i_reshape[start:start + 100, stop:stop + 80, :] = cluster

    data_dict['image'] = i_reshape
